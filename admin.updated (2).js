...
function buildAutoGenTargetRow(initial = {}) {
  ...
  const subtypeValue = initial.subtype || "salud_publica";
...
async function runAutoGenerator() {
  ...
  await ensureBankLoadedForGenerator(Math.min(400, totalTarget * 4));
  ...
  if (!selected.length) {
    alert("No se encontraron casos que cumplan tus criterios.");
    return;
  }
...
function initAutoGeneratorUI() {
  ...
  if (!autoGenTargetsWrap.querySelector(".auto-gen-row")) {
    const first = buildAutoGenTargetRow({ specialty: Object.keys(SPECIALTIES)[0], subtype: "salud_publica", count: 10 });
  ...
  autoGenBtnAddTarget.addEventListener("click", () => {
    const row = buildAutoGenTargetRow({ specialty: Object.keys(SPECIALTIES)[0], subtype: "salud_publica", count: 10 });
