...
function pushTopicHistory(topicId) {
  try {
    const id = String(topicId || "");
    if (!id) return;
    const st = history.state || {};
    // Evitar duplicar
    if (st && st.resources && String(st.resources.topicId || "") === id) return;

    const url = new URL(location.href);
    url.hash = `resources=${encodeURIComponent(id)}`;
    history.pushState({ ...st, resources: { topicId: id } }, "", url.toString());
  } catch {}
}
...
function clearTopicHistory() {
  try {
    ...
    if (st && st.resources) {
      const url = new URL(location.href);
      url.hash = "";
      history.pushState({ ...st, resources: null }, "", url.toString());
    }
  } catch {}
}
...
function applyHistoryState() {
  try {
    const st = history.state || {};
    const id = st && st.resources && st.resources.topicId ? String(st.resources.topicId) : "";
    if (id) {
      if (String(_selectedTopicId || "") !== id) {
  ...
  if (listEl && !listEl.dataset.bound) {
    ...
    listEl.addEventListener("click", async (e) => {
      ...
      if (target?.closest?.("#student-resources-back")) {
        e.preventDefault();
        // Si hay historial, usamos "atrás" real
        try {
          const st = history.state || {};
          if (st && st.resources && st.resources.topicId) {
            history.back();
            return;
          }
        } catch {}
    ...
    <div class="card" style="margin-top:12px;">
      ...
      >
        ...
        <button
          ...
          type="button"
          title="Cerrar"
          style="position:absolute; left:10px; top:10px; z-index:4; border:0; border-radius:10px; padding:10px 12px; background:rgba(17,24,39,.85); color:#fff; font-weight:800; cursor:pointer;"
        >✕</button>
