...
import { getFirestore, initializeFirestore, collection, getDocs, getDoc, doc, query, orderBy } from "https://www.gstatic.com/firebasejs/11.0.1/firebase-firestore.js";
...
function ensureResourcesDb() {
  ...
  try {
  resourcesDb = initializeFirestore(app, {
    experimentalForceLongPolling: true,
    useFetchStreams: false,
  });
} catch (e) {
  console.warn("[resources] initializeFirestore fallback -> getFirestore()", e);
...
/****************************************************
 * ✅ Validación ligera de PDFs locales (/pdfs/*)
 * Evita "MissingPDFException" cuando el link del tema apunta
 * a un archivo que NO existe en GitHub Pages.
 ****************************************************/
const _sr_headCache = new Map(); // url(sin ?v) -> boolean

function _sr_absUrl(u) {
  try {
...
function _sr_stripVersionParam(u) {
  try {
    const url = new URL(String(u || ""), window.location.href);
...
function _sr_localPdfVariants(u) {
  // Solo variantes de nombre (mayúsculas/minúsculas) para /pdfs/
  try {
    const url = new URL(String(u || ""), window.location.href);
    ...
    candidates.add(url.origin + url.pathname); // original
    candidates.add(url.origin + folder + base.toLowerCase() + ".pdf");
    candidates.add(url.origin + folder + base.toUpperCase() + ".pdf");
    // Variante "TitleCase" simple: primera letra mayúscula, resto minúscula
    if (base.length > 0) {
      candidates.add(
        url.origin + folder + (base[0].toUpperCase() + base.slice(1).toLowerCase()) + ".pdf"
      );
    }
...
async function _sr_headOk(u) {
  const key = _sr_stripVersionParam(_sr_absUrl(u));
  if (!key) return true;
  if (_sr_headCache.has(key)) return _sr_headCache.get(key);

  // Timeout corto: si falla por red/CORS, asumimos OK para no bloquear la UI
  const controller = new AbortController();
  const timer = setTimeout(() => controller.abort(), 2500);

  try {
    const res = await fetch(key, { method: "HEAD", cache: "no-store", signal: controller.signal });
    const ok = !!(res && res.ok);
    _sr_headCache.set(key, ok);
    return ok;
  } catch {
    _sr_headCache.set(key, true);
    return true;
  } finally {
    clearTimeout(timer);
  }
...
if (rawUrl) {
  const storagePath = topic?.previewPdf?.path || "";

  // Si es un PDF local (/pdfs/*), validamos que exista para evitar MissingPDFException.
  let finalUrl = rawUrl;

  if (_sr_isLocalPdfs(rawUrl)) {
    let ok = await _sr_headOk(rawUrl);

    if (!ok) {
      // Intentar variantes comunes de nombre (mayúsculas/minúsculas)
      const variants = _sr_localPdfVariants(rawUrl);
      for (const v of variants) {
        if (await _sr_headOk(v)) {
          finalUrl = v;
          ok = true;
          break;
        }
    ...
    // Si NO existe en /pdfs/ y hay path legacy en Storage, usarlo como fallback
    if (!ok && storagePath) {
      const url = await getPreviewPdfDownloadUrl(storagePath);
    ...
    // Si no hay fallback, avisar claramente
    if (!ok) {
      _selectedPreviewUrl = "";
      showPreviewUnavailableNote("No se encontró el PDF en /pdfs/. Revisa el nombre/ruta en Admin o vuelve a subir el archivo.");
      return;
    }
  }

  const withV = appendVersionParam(finalUrl, version);
...
function makeLocalPdfViewerUrl(pdfUrl) {
  ...
  viewer.searchParams.set("z", "3");
  // 🔒 Bypass de cache para el visor: el Service Worker cachea pdf-viewer.html.
  // Si actualizamos el visor y el usuario tiene la versión anterior en cache,
  // esto fuerza a pedir una URL nueva.
  viewer.searchParams.set("v", "20260208-1");
  ...
  if (listEl && !listEl.dataset.bound) {
    ...
    listEl.addEventListener("click", async (e) => {
      ...
      if (target?.closest?.("#student-resources-back")) {
        ...
        // Cerrar el detalle inmediatamente (evita doble clic)
        disposeInlinePreview();
        _selectedTopicId = null;
        _selectedPreviewUrl = "";
        _selectedOpenUrl = "";
        ...
        // Normalizar URL/estado a vista lista
        try {
          __resourcesPushingState = true;
          const u = new URL(location.href);
          u.hash = "";
          history.replaceState({ ...(history.state || {}), __resources: true, view: "list" }, "", u.toString());
        } catch {} finally {
          __resourcesPushingState = false;
        }
        return;
