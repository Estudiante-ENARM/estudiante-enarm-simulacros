...
(async function bootstrapAdmin() {
  try {
    ...
    const { getStorage, ref: storageRef, uploadBytes, deleteObject, getDownloadURL } = storageMod;
...
// ✅ PDF para vista previa (Cloud Storage)
const resPreviewPdfWrap = document.getElementById("admin-resources-preview-pdf-wrap");
const resPreviewPdfInput = document.getElementById("admin-resources-preview-pdf-input");
const resPreviewPdfStatus = document.getElementById("admin-resources-preview-pdf-status");
const resPreviewPdfBtnChoose = document.getElementById("admin-resources-preview-pdf-btn-choose");
...
const PREVIEW_PDF_CACHE_SECONDS = 25920000;
const PREVIEW_PDF_CACHE_CONTROL = `public, max-age=${PREVIEW_PDF_CACHE_SECONDS}, immutable`;

let _resourcesStorage = null;

function ensureResourcesStorage() {
  if (_resourcesStorage) return _resourcesStorage;
  // inicializa resourcesApp si hace falta
  ensureResourcesDb();
  _resourcesStorage = getStorage(_resourcesApp);
  return _resourcesStorage;
}

function buildPreviewPdfPath(topicId, version) {
  const tid = String(topicId || "").replace(/[^a-zA-Z0-9_-]/g, "_");
  const v = Number(version) || 1;
  return `preview_pdfs/${tid}/v${v}.pdf`;
}

// Estado UI PDF vista previa (existente vs pendiente)
let _resourcesPreviewPdfExisting = null; // {path, version, bytes, updatedAt}
let _resourcesPreviewPdfPending = null;  // idem, pero aún no guardado en Firestore
let _resourcesPreviewSelectedFile = null;
let _resourcesPreviewDeleteArmed = false;
let _resourcesPreviewDeleteArmTimer = null;
...
function setResEditorEnabled(enabled) {
  ...
  // PDF vista previa
  setPreviewPdfControlsEnabled(!!enabled);
}
...
/****************************************************
 * PDF VISTA PREVIA (Cloud Storage) - UI helpers
 ****************************************************/
function renderResourcesPreviewPdfStatus() {
  if (!resPreviewPdfStatus) return;

  const existing = _resourcesPreviewPdfExisting;
  const pending = _resourcesPreviewPdfPending;

  const parts = [];
  if (!_resourcesSelectedId) {
    parts.push("Selecciona un tema para gestionar su PDF de vista previa.");
  } else {
    if (existing?.path) {
      parts.push(`Actual: v${existing.version || "?"} (${existing.bytes ? formatBytes(existing.bytes) : "—"})`);
    } else {
      parts.push("Actual: — (no hay PDF)");
    }

    if (_resourcesPreviewSelectedFile) {
  ...
  resPreviewPdfStatus.textContent = parts.join(" • ");
}

function setPreviewPdfControlsEnabled(enabled) {
  const hasTopic = !!_resourcesSelectedId;
  const e = !!enabled && hasTopic;
...
function clearPreviewPdfDraft() {
  _resourcesPreviewSelectedFile = null;
  _resourcesPreviewPdfPending = null;
  if (resPreviewPdfInput) resPreviewPdfInput.value = "";
  renderResourcesPreviewPdfStatus();
}

function loadPreviewPdfFromTopic(topic) {
  _resourcesPreviewPdfExisting = (topic && typeof topic.previewPdf === "object" && topic.previewPdf) ? topic.previewPdf : null;
  clearPreviewPdfDraft();
  setPreviewPdfControlsEnabled(true);
  renderResourcesPreviewPdfStatus();
}

async function uploadPreviewPdfSelectedFile() {
  if (!_resourcesSelectedId) {
    alert("Selecciona un tema antes de subir un PDF.");
    return;
  }
  const file = _resourcesPreviewSelectedFile;
  if (!file) {
  ...
  const existingVersion = Number(_resourcesPreviewPdfExisting?.version) || 0;
  const newVersion = existingVersion + 1;
  const newPath = buildPreviewPdfPath(_resourcesSelectedId, newVersion);

  const storage = ensureResourcesStorage();
  const objRef = storageRef(storage, newPath);

  setLoadingButton(resPreviewPdfBtnUpload, true, "Subir / Reemplazar");
  try {
    const meta = {
      contentType: "application/pdf",
      cacheControl: PREVIEW_PDF_CACHE_CONTROL,
    };
    const snap = await uploadBytes(objRef, file, meta);
    const bytes = snap?.metadata?.size || file.size || null;
    ...
    console.error("Error subiendo PDF vista previa:", err);
...
async function savePreviewPdfToTopic() {
  if (!_resourcesSelectedId) {
    alert("Selecciona un tema antes de guardar.");
    return;
  }
  if (!_resourcesPreviewPdfPending?.path) {
    alert("No hay un PDF pendiente por guardar. Primero súbelo con 'Subir / Reemplazar'.");
    return;
  }
  ...
  const prevPath = _resourcesPreviewPdfExisting?.path || null;
  const next = {
    path: _resourcesPreviewPdfPending.path,
    version: _resourcesPreviewPdfPending.version || 1,
    bytes: _resourcesPreviewPdfPending.bytes || null,
    updatedAt: serverTimestamp(),
  };

  setLoadingButton(resPreviewPdfBtnSave, true, "Guardar PDF");
  try {
    await setDoc(topicRef, { previewPdf: next }, { merge: true });

    _resourcesPreviewPdfExisting = { ...next, updatedAt: Date.now() };
    _resourcesPreviewPdfPending = null;
    _resourcesPreviewSelectedFile = null;
    if (resPreviewPdfInput) resPreviewPdfInput.value = "";

    if (prevPath && prevPath !== next.path) {
      try {
        const storage = ensureResourcesStorage();
        await deleteObject(storageRef(storage, prevPath));
      } catch (err) {
        console.warn("No se pudo borrar versión anterior de PDF (no crítico):", err);
      }
    }

    const idx = _resourcesTopics.findIndex((t) => t.id === String(_resourcesSelectedId));
    if (idx >= 0) _resourcesTopics[idx].previewPdf = next;

    renderResourcesPreviewPdfStatus();
    alert("PDF guardado en el tema.");
  } catch (err) {
    console.error("Error guardando previewPdf en tema:", err);
...
async function deletePreviewPdfFromTopic() {
  if (!_resourcesSelectedId) return;

  if (!_resourcesPreviewDeleteArmed) {
    _resourcesPreviewDeleteArmed = true;
    ...
    _resourcesPreviewDeleteArmTimer = setTimeout(() => {
      _resourcesPreviewDeleteArmed = false;
  ...
  const existingPath = _resourcesPreviewPdfExisting?.path;
  if (!existingPath) {
    clearPreviewPdfDraft();
  ...
  try {
    ...
    await setDoc(topicRef, { previewPdf: null }, { merge: true });

    const idx = _resourcesTopics.findIndex((t) => t.id === String(_resourcesSelectedId));
    if (idx >= 0) _resourcesTopics[idx].previewPdf = null;

    _resourcesPreviewPdfExisting = null;
    clearPreviewPdfDraft();
    renderResourcesPreviewPdfStatus();
    alert("PDF eliminado del tema.");
  } catch (err) {
    console.error("Error eliminando previewPdf:", err);
...
// Bind eventos (una sola vez)
  ...
  // ✅ PDF vista previa (Cloud Storage)
  resPreviewPdfBtnChoose?.addEventListener("click", () => {
    if (!resPreviewPdfInput) return;
    resPreviewPdfInput.click();
  });

  resPreviewPdfInput?.addEventListener("change", () => {
    const file = resPreviewPdfInput?.files?.[0] || null;
    _resourcesPreviewSelectedFile = file;
    renderResourcesPreviewPdfStatus();
  });

  resPreviewPdfBtnUpload?.addEventListener("click", async () => {
    await uploadPreviewPdfSelectedFile();
  });

  resPreviewPdfBtnSave?.addEventListener("click", async () => {
    await savePreviewPdfToTopic();
  });

  resPreviewPdfBtnDelete?.addEventListener("click", async () => {
    await deletePreviewPdfFromTopic();
  });
...
onAuthStateChanged(auth, async (user) => {
  ...
  try {
    ...
    // ✅ Reordenamiento sidebar: sincroniza desde Firestore (si existe)
    try {
      const remoteOrder = await readAdminNavOrderRemote();
      if (remoteOrder && remoteOrder.length) {
        applyAdminNavOrder(remoteOrder);
...
// Persistencia: localStorage + Firestore (changes/admin_nav_order)
const ADMIN_NAV_ORDER_LOCAL_KEY = "admin_nav_order_v1";
const DEFAULT_ADMIN_NAV_ORDER = ["exams","bank","mini","users","analytics","landing","resources","import_export"];

const adminNavListEl = document.getElementById("admin-nav-list") || btnNavExams?.parentElement || null;

function getAdminNavButtonByKey(key) {
  const map = {
    exams: btnNavExams,
    bank: btnNavBank,
    mini: btnNavMini,
    ...
    landing: btnNavLanding,
    resources: btnNavResources,
    import_export: btnNavImportExport,
  };
  return map[key] || null;
}

function readAdminNavOrderLocal() {
  try {
    const raw = localStorage.getItem(ADMIN_NAV_ORDER_LOCAL_KEY);
...
function writeAdminNavOrderLocal(order) {
  try {
    localStorage.setItem(ADMIN_NAV_ORDER_LOCAL_KEY, JSON.stringify(order || []));
  } catch {}
}

async function readAdminNavOrderRemote() {
  try {
    const refDoc = doc(db, "changes", "admin_nav_order");
    const snap = await getDoc(refDoc);
    if (!snap.exists()) return null;
    const data = snap.data() || {};
    if (Array.isArray(data.order) && data.order.length) return data.order.filter(Boolean);
  } catch (err) {
...
async function writeAdminNavOrderRemote(order) {
  try {
    const refDoc = doc(db, "changes", "admin_nav_order");
    await setDoc(refDoc, { order: order || DEFAULT_ADMIN_NAV_ORDER, updatedAt: serverTimestamp() }, { merge: true });
  } catch (err) {
...
function normalizeAdminNavOrder(order) {
  const base = DEFAULT_ADMIN_NAV_ORDER.slice();
  const out = [];
  const seen = new Set();
...
function applyAdminNavOrder(order) {
  if (!adminNavListEl) return;
  const norm = normalizeAdminNavOrder(order);
  norm.forEach((k) => {
    const el = getAdminNavButtonByKey(k);
    if (el) adminNavListEl.appendChild(el);
  });
}

function currentAdminNavOrderFromDom() {
  if (!adminNavListEl) return DEFAULT_ADMIN_NAV_ORDER.slice();
  const btns = Array.from(adminNavListEl.querySelectorAll("button.sidebar-btn"));
  const keyById = {
    "admin-btn-nav-exams": "exams",
    "admin-btn-nav-bank": "bank",
    "admin-btn-nav-mini": "mini",
    "admin-btn-nav-users": "users",
    "admin-btn-nav-analytics": "analytics",
    "admin-btn-nav-landing": "landing",
    "admin-btn-nav-resources": "resources",
    "admin-btn-nav-import-export": "import_export",
  };
  const out = btns.map((b) => keyById[b.id]).filter(Boolean);
  return normalizeAdminNavOrder(out);
}

function initAdminNavReorderHandlers() {
  if (!adminNavListEl) return;
  ...
  const buttons = Array.from(adminNavListEl.querySelectorAll("button.sidebar-btn"));
  buttons.forEach((btn) => {
    const key = keyById[btn.id];
    if (!key) return;
    btn.draggable = true;
  ...
  adminNavListEl.addEventListener("dragover", (e) => {
    e.preventDefault();
    const dragging = adminNavListEl.querySelector(".dragging");
    if (!dragging) return;

    const afterElement = getDragAfterElement(adminNavListEl, e.clientY);
    if (afterElement == null) {
      adminNavListEl.appendChild(dragging);
    } else {
      adminNavListEl.insertBefore(dragging, afterElement);
    }
  });

  adminNavListEl.addEventListener("drop", async (e) => {
    e.preventDefault();
    const order = currentAdminNavOrderFromDom();
    writeAdminNavOrderLocal(order);
    await writeAdminNavOrderRemote(order);
  });
}
...
// Aplica orden guardado localmente (rápido, antes de cargar Firestore)
applyAdminNavOrder(readAdminNavOrderLocal() || DEFAULT_ADMIN_NAV_ORDER);
