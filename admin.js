...
// ✅ util: identifica si un doc de "questions" es un caso válido para el banco/buscadores
// Nota: en tu data, muchos casos del banco SÍ traen examId. Por eso NO usamos examId para filtrar.
// Consideramos caso válido si trae caseText y arreglo questions.
function isBankCaseDoc(data) {
  const hasCaseText = typeof data?.caseText === "string" && data.caseText.trim().length > 0;
  const hasQuestions = Array.isArray(data?.questions);
  return hasCaseText && hasQuestions;
}
...
function renderBankItem(docId, data) {
  ...
  const isBankDoc = isBankCaseDoc(data);
