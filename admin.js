...
/****************************************************
 * ✅ VISTA PREVIA (PDF CARGADO EN FIREBASE STORAGE)
 * - Se inserta debajo de Links / PDFs (solo en Resúmenes y GPC)
 * - El alumno lo verá en la vista previa; los links quedan en el botón "Abrir PDF"
 ****************************************************/
...
function ensureResourcesPreviewPdfUi() {
  ...
  box.id = "admin-resources-preview-pdf-box";
  box.className = "card";
  ...
  box.innerHTML = `
    <div style="display:flex;justify-content:space-between;align-items:center;gap:10px;flex-wrap:wrap;">
      <div>
        <div style="font-weight:700;font-size:13px;">Vista previa (PDF cargado)</div>
        <div class="panel-subtitle" id="admin-resources-preview-pdf-status">Selecciona un tema.</div>
      </div>

      <div class="flex-row" style="gap:8px;justify-content:flex-end;">
        <a id="admin-resources-preview-pdf-open" class="btn btn-outline btn-sm hidden" target="_blank" rel="noopener">Abrir</a>
        <button id="admin-resources-preview-pdf-delete" class="btn btn-secondary btn-sm" type="button" disabled>Eliminar</button>
      </div>
    </div>

    <label class="field" style="margin-top:10px;">
      <span>Subir o reemplazar PDF (solo PDF)</span>
      <input id="admin-resources-preview-pdf-file" type="file" accept="application/pdf" />
    </label>

    <div class="flex-row" style="justify-content:flex-end;gap:8px;margin-top:10px;">
      <button id="admin-resources-preview-pdf-upload" class="btn btn-primary btn-sm" type="button" disabled>Guardar PDF</button>
    </div>

    <div class="panel-subtitle" style="margin-top:8px;">
      El estudiante verá este PDF en <b>Vista previa</b>. Tus <b>Links / PDFs</b> permanecen intactos para el botón <b>Abrir PDF</b>.
      <br/>El PDF se sube con caché (~10 meses) y versión automática (v1, v2, v3...) para que cuando lo actualices se descargue el nuevo.
    </div>
  `;

  // Inserta justo antes del bloque de Repaso/Mini-examen
  ...
  const openLink = box.querySelector("#admin-resources-preview-pdf-open");
  const delBtn = box.querySelector("#admin-resources-preview-pdf-delete");
  const fileInput = box.querySelector("#admin-resources-preview-pdf-file");
  const uploadBtn = box.querySelector("#admin-resources-preview-pdf-upload");

  _resPreviewPdfUi = { box, status, openLink, delBtn, fileInput, uploadBtn };
...
function renderResourcesPreviewPdfUi(topic) {
  ...
  const url = topic?.previewPdfUrl || "";
  const ver = Number(topic?.previewPdfVersion || 0) || 0;
  const when = formatMaybeDate(topic?.previewPdfUpdatedAt);

  if (!hasTopic) {
    ui.status.textContent = "Selecciona un tema.";
  } else if (!isAuthed) {
    ui.status.textContent = "Inicia sesión en Biblioteca para subir/eliminar el PDF de vista previa.";
  } else if (url) {
    ui.status.textContent = `PDF cargado (v${ver || 1})${when ? " · " + when : ""}`;
  } else {
    ui.status.textContent = "Aún no hay PDF de vista previa para este tema.";
  }

  if (url) {
    ui.openLink.classList.remove("hidden");
    ui.openLink.href = url;
  } else {
    ui.openLink.classList.add("hidden");
    ui.openLink.removeAttribute("href");
  }

  ui.uploadBtn.disabled = !(hasTopic && isAuthed);
  ui.delBtn.disabled = !(hasTopic && isAuthed && !!url);
  ui.fileInput.disabled = !(hasTopic && isAuthed);
}
...
async function adminResourcesUploadPreviewPdf() {
  ...
  const u = ensureResourcesAuth()?.currentUser;
  if (!u) {
    alert("Primero inicia sesión en Biblioteca para poder subir el PDF.");
    return;
  }
  ...
  const nextVer = (Number(topic.previewPdfVersion || 0) || 0) + 1;
  const ver = Math.max(1, nextVer);

  const path = `topic_previews/${_resourcesSelectedId}/v${ver}.pdf`;

  const metadata = {
    contentType: "application/pdf",
    cacheControl: `public,max-age=${RESOURCES_PDF_CACHE_MAX_AGE_SECONDS}`,
  };

  setLoadingButton(ui.uploadBtn, true, "Guardar PDF");
  try {
    const uploaded = await tryUploadWithBucketFallback(path, file, metadata);
    ...
    alert("PDF de vista previa guardado.");
  } catch (err) {
    console.error(err);
    alert("No se pudo subir el PDF. Revisa: sesión de biblioteca, reglas de Storage/Firestore y bucket.");
  } finally {
    setLoadingButton(ui.uploadBtn, false, "Guardar PDF");
  }
}
...
async function adminResourcesDeletePreviewPdf() {
  ...
  const topic = _resourcesTopics.find((t) => t.id === _resourcesSelectedId) || {};
  if (!topic.previewPdfUrl && !topic.previewPdfPath) {
    alert("Este tema no tiene PDF de vista previa.");
    return;
  }

  const ok = confirm("¿Eliminar el PDF de vista previa de este tema?");
  if (!ok) return;

  setLoadingButton(ui.delBtn, true, "Eliminar");
  try {
    if (topic.previewPdfPath) await tryDeleteWithBucketFallback(topic.previewPdfPath);
    ...
    alert("PDF de vista previa eliminado.");
  } catch (err) {
    console.error(err);
    alert("No se pudo eliminar el PDF de vista previa.");
  } finally {
    setLoadingButton(ui.delBtn, false, "Eliminar");
  }
}
