...
async function hydrateSelectedTopicPreviewPdf(topic) {
  ...
  if (rawUrl) {
    ...
    const resolved = await resolveLocalPdfUrl(withV);
    const viewer = makePreviewUrl(resolved);
...
const DEFAULT_PDF_RENDER_SCALE = 3.0; // 300% (mejor nitidez)
...
// --- Navegación por hash para que el botón ATRÁS funcione (desktop + móvil)
function getResourcesHashId() {
  const h = String(window.location.hash || "").replace(/^#/, "");
  if (!h) return null;
  if (h === "resources") return null;
  if (h.startsWith("resources=")) return decodeURIComponent(h.split("=").slice(1).join("=")) || null;
  return null;
}

function setResourcesHashId(id) {
  if (!id) {
    // lista
    window.location.hash = "resources";
  } else {
    window.location.hash = `resources=${encodeURIComponent(id)}`;
  }
}
...
function makeDriveDirectPdfUrl(fileId) {
  if (!fileId) return "";
  // Intento de URL directa a PDF (sin toolbar de Drive). Puede fallar si Drive bloquea CORS.
...
function makeLocalPdfViewerUrl(pdfUrl) {
  ...
  viewer.searchParams.set("z", String(DEFAULT_PDF_RENDER_SCALE));
  return viewer.toString();


async function resolveLocalPdfUrl(absPdfUrl) {
  const url = toAbsoluteUrl(absPdfUrl);
  if (!url) return absPdfUrl;
  // Solo para PDFs servidos desde esta misma GitHub Pages (carpeta /pdfs/)
  try {
    const u = new URL(url);
    if (!u.pathname.includes("/pdfs/")) return url;
    // Probamos existencia con una request mínima
    const r = await fetch(url, { method: 'GET', headers: { Range: 'bytes=0-0' }, cache: 'no-cache' });
    if (r && (r.status === 200 || r.status === 206)) return url;
    if (r && r.status !== 404) return url;
  } catch { return url; }

  // Fallbacks comunes (typos)
  const candidates = [];
  candidates.push(url.replace(/Appendicitis/gi, 'Apendicitis'));
  candidates.push(url.replace(/Apendicitis/gi, 'Appendicitis'));

  for (const c of candidates) {
    if (c === url) continue;
    try {
      const r2 = await fetch(c, { method: 'GET', headers: { Range: 'bytes=0-0' }, cache: 'no-cache' });
      if (r2 && (r2.status === 200 || r2.status === 206)) return c;
    } catch {}
  }
  return url;
}
...
function makePreviewUrl(url) {
  ...
  // Drive: intentamos URL directa -> visor local (sin toolbar de Drive)
  if (n.includes("drive.google.com")) {
    const id = extractDriveFileId(normalized);
    const direct = id ? makeDriveDirectPdfUrl(id) : "";
    return direct ? makeLocalPdfViewerUrl(direct) : "";
  }
  ...
  // Otros formatos: no se previsualizan aquí
  return "";
}
...
function makeOpenUrl(url) {
  ...
  // PDFs (incluye Drive): abrir con visor local para evitar toolbars con descarga/impresión
  if (n.includes("drive.google.com")) {
    const id = extractDriveFileId(normalized);
    const direct = id ? makeDriveDirectPdfUrl(id) : "";
    return direct ? makeLocalPdfViewerUrl(direct) : normalized;
  }
...
function cachePreviewDomRefs() {
  ...
  // Mensajes desde pdf-viewer.html (cuando termina de renderizar)
  if (!window.__studentResourcesPdfMsgBound) {
    window.__studentResourcesPdfMsgBound = true;
    window.addEventListener('message', (ev) => {
      const d = ev && ev.data;
      if (!d || typeof d !== 'object') return;
      if (d.type === 'pdf-rendered') setPreviewLoading(false);
      if (d.type === 'pdf-error') setPreviewLoading(false);
    });
  }

  // Hash navigation: que ATRÁS regrese a la lista y no congele
  if (!window.__studentResourcesHashBound) {
    window.__studentResourcesHashBound = true;
    window.addEventListener('hashchange', () => {
      const id = getResourcesHashId();
      if (!id) {
        if (_selectedTopicId) {
          _selectedTopicId = null;
          setResourcesHashId(null);
          disposeInlinePreview();
          scheduleRender(true);
        }
        return;
      }
      // abrir tema desde hash
      if (String(_selectedTopicId || '') !== String(id)) {
        _selectedTopicId = id;
        setResourcesHashId(id);
        scheduleRender(true);
      }
    });
  }
  ...
  if (_previewFrameEl && !_previewFrameEl.dataset.bound) {
    ...
    _previewFrameEl.addEventListener("load", () => {
      ...
      // ✅ Fix "preview en blanco" (móvil): verificamos que el visor responda.
      // Si no responde, forzamos un re-render del iframe una sola vez por token.
      try {
        ...
        const token = String(_previewFrameEl.dataset.token || "");
        const retryKey = "retried_" + token;
        if (_previewFrameEl.dataset[retryKey] === "1") return;

        setTimeout(() => {
          pingPdfViewer(500).then((ok) => {
            if (ok) {
              postPdfViewerMessage("pdfViewer:refresh");
              return;
            }
            // marcar retry y forzar recarga
            _previewFrameEl.dataset[retryKey] = "1";
            schedulePreviewUpdate(true);
          });
        }, 120);
      } catch {}
