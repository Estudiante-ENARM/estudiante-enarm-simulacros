...
const btnExamsSidebar = document.getElementById("estudiante-examenes-btn");
...
// Encabezado
const studentUserEmailSpan = document.getElementById("correo-electrónico-del-usuario-estudiante");
...
const examSubtitle = document.getElementById("subtítulo-examen-estudiante");
 ...
 ****************************************************/
deje que isRestoringState = falso;
...
onAuthStateChanged(auth, async (usuario) => {
  ...
  intentar {
    ...
    // ✅ Sincroniza el progreso Biblioteca (Firestore → localStorage) para multidispositivo
    esperar startResourcesCloudSync({ escuchar: verdadero });
...
si (btnCerrar sesión) {
  btnLogout.addEventListener("clic", async () => {
    intentar {
      // Limpia oyente de progreso Biblioteca
      intentar {
        si (tipo de _resourcesCloudUnsub === "función") _resourcesCloudUnsub();
      } atrapar {}
...
si (btnExamsSidebar) {
  btnExamsSidebar.addEventListener("clic", () => {
    // 1) Asegura estar en la vista de solicitudes
    si (currentView !== "sección") {
      switchToSectionView({ restaurar: verdadero }); // evita tocar el estado guardado
    }

    // 2) Alternar menú de secciones
 ...
 ****************************************************/
deje que _resourcesCloudSyncStarted = falso;
deje que _resourcesCloudUnsub = null;
deje que _resourcesCloudCompleted = nuevo Conjunto();
deje que _resourcesCloudPushTimer = null;

función _resourcesProgressDocRef() {
  si (!currentUser || !currentUser.email) devuelve nulo;
  // 🔒 Mantiene consistencia con el patrón ya usado en examAttempts
  devolver doc(db, "usuarios", usuarioActual.email, "recursosProgreso", "v1");
}

función _resourcesLocalStorageKey() {
  // Importante: debe coincidir con la clave usada por la UI de biblioteca actual
  constante userKey = _resourcesUserKey();
  devolver `recursos_completados_${userKey}`;
}

función _writeResourcesCompletedToLocalStorage(setOrArr) {
  const arr = Array.isArray(setOrArr)? setOrArr : Array.from(setOrArr || []);
  correo electrónico constante =
    (usuarioActual && (usuarioActual.correoElectrónico || usuarioActual.uid)) ? String(usuarioActual.correoElectrónico || usuarioActual.uid) : "";
  constante userKey = _resourcesUserKey();

  const keys = [`resources_completed_${userKey}`];
  si (correo electrónico) {
    keys.push(`recursos_completados_${normalizeText(correo electrónico)}`);
    keys.push(`recursos_completados_${encodeURIComponent(correo electrónico)}`);
  }

  const uniqueKeys = Array.from(new Set(claves));

  para (clave constante de uniqueKeys) {
    intentar {
      localStorage.setItem(clave, JSON.stringify(arr));
    } captura (e) {
      console.warn("No se pudo guardar el progreso de la biblioteca en localStorage:", e);
    }
  }
}

función _normalizeCompletedArray(arr) {
  si (!Array.isArray(arr)) devuelve [];
  regreso arr
    .map((x) => (x == null ? "" : String(x)).trim())
    .filtro(Booleano);
}

función asíncrona startResourcesCloudSync({ listen = true } = {}) {
  si (_resourcesCloudSyncStarted) retorna;
  si (!usuarioActual || !usuarioActual.email) devolver;

  _resourcesCloudSyncStarted = verdadero;

  constante ref = _resourcesProgressDocRef();
  si (!ref) retorna;

  constante aplicarRemoto = (arrRemoto) => {
    constante clean = _normalizeCompletedArray(remoteArr);
    _resourcesCloudCompleted = nuevo Conjunto(limpio);

    // ✅ Refleja remota en local (así la UI existente se "entera")
    _writeResourcesCompletedToLocalStorage(limpio);
  ...
  intentar {
    ...
    constante datos = snap.exists() ? snap.data() : {};
    const remoteArr = _normalizeCompletedArray(datos.completed || []);

    si (remoteArr.length > 0) {
      // ✅ Firestore ya tiene datos → úsala como fuente de verdad
      aplicarRemoto(remoteArr);
    } demás {
      // Firestore vacío / doc nuevo → conserva local si existe, y súbelo
      constante localSet = _readCompletedResourcesSet();
      si (localSet.size > 0) {
        constante localArr = Array.from(localSet);
        _resourcesCloudCompleted = nuevo Conjunto(localArr);
        esperar setDoc(
          árbitro,
          { completado: localArr, actualizadoEn: serverTimestamp() },
          { fusionar: verdadero }
        );
      } demás {
        aplicarRemoto([]);
    ...
    console.warn("No se pudo cargar el progreso de la biblioteca desde Firestore:", e);
  }

  // 2) Oyente en vivo (opcional, pero útil si tienes abierto móvil + escritorio)
  si (!escuchar) retorna;

  intentar {
    _resourcesCloudUnsub = enSnapshot(
      árbitro,
      (chasquido) => {
        constante datos = snap.exists() ? snap.data() : {};
        applyRemote(datos.completado || []);
      },
      (err) => {
...
función asíncrona pushResourcesProgressToCloud() {
  constante ref = _resourcesProgressDocRef();
  si (!ref) retorna;
  ...
  constante cloudSig = Array.from(_resourcesCloudCompleted).sort().join("||");
  si (localSig === cloudSig) retorna;

  // Optimista: actualiza caché local de remoto
  _resourcesCloudCompleted = nuevo Conjunto(localArr);
  ...
  intentar {
    ...
    console.warn("No se pudo sincronizar el progreso de la biblioteca a Firestore:", e);
  }
}

función programarRecursosProgresoCloudPush() {
  si (_resourcesCloudPushTimer) clearTimeout(_resourcesCloudPushTimer);
  _resourcesCloudPushTimer = setTimeout(() => {
    empujarRecursosProgresoALaNube();
  }, 250);
}
 ...
 ****************************************************/
deje que _resourcesProgressBound = falso;
deje que _resourcesProgressBaseline = { total: 0, hecho: 0 };

función _resourcesUserKey() {
  // Preferimos email para que el progreso sea consistente entre dispositivos
  devolver (usuarioActual && (usuarioActual.email || usuarioActual.uid)) || "anónimo";
}

función _readCompletedResourcesSet() {
  ...
  // ✅ Leemos varias variantes para evitar pérdida de progreso por diferencias de clave entre módulos/versiones
  const keys = [`resources_completed_${userKey}`];
  si (correo electrónico) {
    keys.push(`recursos_completados_${normalizeText(correo electrónico)}`);
    keys.push(`recursos_completados_${encodeURIComponent(correo electrónico)}`);
  ...
  constante ingesta = (arr) => {
    si (!Array.isArray(arr)) retorna;
    para (elemento constante de arr) {
      si (tipo de elemento === "cadena") {
        establecer.add(item.trim());
        continuar;
      }
      si (elemento && tipo de elemento === "objeto") {
        ...
        para (const c de candidatos) set.add(String(c).trim());
      }
    }
  };
...
función _extractTopicKeyFromCard(tarjeta) {
  si (!card) devuelve nulo;

  const ds = tarjeta.conjunto de datos || {};
  const candidatos = [
    ds.Id.de tema,
    ds.docId,
    ds.id,
    clave ds,
    card.getAttribute("id del tema de datos"),
    tarjeta.getAttribute("id-del-documento-de-datos"),
    tarjeta.getAttribute("id-de-datos"),
    card.getAttribute("clave-de-datos"),
  ].filtro(Booleano);

  si (candidatos.longitud) devuelve String(candidatos[0]).trim();

  // Respaldo: usa el título visible (si no hay conjunto de datos)
  constante títuloEl =
    card.querySelector("h3, h4, .título-del-recurso, .título-de-la-tarjeta, .título-del-tema") ||
    card.querySelector("[título-de-datos]");
  si (títuloEl) {
...
función _getVisibleResourceTopicKeys(listaEl) {
  si (!listEl) devuelve nuevo Conjunto();
  // Intentamos capturar tarjetas/ítems típicos
  candidatos constantes = listEl.querySelectorAll(
    "[id-tema-datos],[id-doc-datos],[id-datos],[clave-datos],.tarjeta-recurso,.tarjeta"
  );
  constantes claves = nuevo Conjunto();
  candidatos.paraCada((el) => {
...
función _setResourcesProgressUI(hecho, total) {
  const textEl = document.getElementById("texto-de-progreso-de-recursos-para-estudiantes");
  const barEl = document.getElementById("barra-de-progreso-de-recursos-para-estudiantes");
  si (!textEl || !barEl) retorna;

  constante safeTotal = Math.max(0, Número(total) || 0);
  constante seguroHecho = Math.max(0, Math.min(seguroTotal, Número(hecho) || 0));
  constante pct = seguroTotal ? Math.round((seguroTerminado / seguroTotal) * 100) : 0;

  textEl.textContent = `${safeDone} de ${safeTotal} (${pct}%)`;
  barEl.style.width = `${pct}%`;
}

función _recomputeResourcesProgressBaseline() {
  const listEl = document.getElementById("lista-de-recursos-para-estudiantes");
  const searchEl = document.getElementById("búsqueda-de-recursos-para-estudiantes");
  si (!listEl) retorna;

  // La barra de progreso debe depender SOLO de la especialidad (no del texto de búsqueda)
  const hasSearch = !!(buscarEl && String(buscarEl.valor || "").trim());
  si (hasSearch) retorna;

  const visibleKeys = _getVisibleResourceTopicKeys(listEl);
  constanteCompletedSet = _readCompletedResourcesSet();

  dejar hecho = 0;
  visibleKeys.forEach((k) => {
    si (completedSet.has(k)) hecho += 1;
  });

  _resourcesProgressBaseline = { total: visibleKeys.size, hecho };
  _setResourcesProgressUI(hecho, visibleKeys.size);
}

función bindStudentResourcesProgressUI() {
  si (_resourcesProgressBound) retorna;
  _resourcesProgressBound = verdadero;

  const listEl = document.getElementById("lista-de-recursos-para-estudiantes");
  const searchEl = document.getElementById("búsqueda-de-recursos-para-estudiantes");
  const specialtyEl = document.getElementById("recursos-para-estudiantes-especialidad");

  si (!listEl) retorna;

  // Observa cambios del listado (carga inicial, marcar completado, cambiar especialidad, etc.)
  const mo = nuevo MutationObserver(() => {
    // Recalcula la línea base solo si NO hay búsqueda activa
    _recomputeResourcesProgressBaseline();
  });
  mo.observe(listEl, { childList: verdadero, subárbol: verdadero });

  // ✅ Si el usuario marca / desmarca completado, sincroniza a Firestore
  // (La UI actual usa localStorage; aquí detectamos cambios y los subimos)
  listEl.addEventListener("haga clic", () => {
    programarRecursosProgresoCloudPush();
  });
  listEl.addEventListener("cambiar", () => {
    programarRecursosProgresoCloudPush();
  });


  // Cuando se cambia de especialidad:
  si (especialidadEl) {
    specialtyEl.addEventListener("cambio", () => {
      si (buscarEl) {
        // Restablecer búsqueda para que el total refleje la especialidad completa
        búsquedaEl.valor = "";
  ...
  // Al borrar la búsqueda manualmente, reactivamos el baseline
  si (buscarEl) {
...
función switchToSectionView(opts = {}) {
  // ✅ Aquí NO se cierra el menú de secciones
  // _examsMenuOpen = falso;
  // establecerSeccionesDeBarraLateralVisible(falso);
...
función asíncrona switchToResourcesView(opts = {}) {
  ...
  intentar {
    ...
    establecerTiempo de espera(() => {
      intenta { _recomputeResourcesProgressBaseline(); } catch (e) {}
    }, 120);
  } atrapar (err) {
  ...
  intentar {
    ...
    intentosSnap.forEach((docSnap) => {
      ...
      si (historyArr.length === 0) {
        examHistoryResults.push({ IdExamen, NombreExamen, IdSección, NombreSección, Puntuación, ÚltimoIntento });
      } demás {
...
función renderProgressChart(examResults) {
  ...
  const datos = ordenados.mapa((r) => (tipo de r.puntaje === "número" ? r.puntaje : 0));
